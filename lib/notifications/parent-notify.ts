...
async function fetchParentContacts(
  ...
  return (data as any[]).map((row) => ({
    ...
    phone_number: row.parents.phone_number ?? null,
...
export async function notifyAbsence(params: AbsenceNotifyParams) {
  ...
  for (const parent of parents) {
    ...
    let smsMeta: Record<string, unknown> = {};
    if (parent.phone_number) {
      const smsResult = await sendSms(parent.phone_number, smsBody);
      smsStatus = smsResult.success ? "sent" : "failed";
      smsMeta = smsResult.messageId
        ? { at_message_id: smsResult.messageId }
  ...
  for (const parent of parents) {
    ...
    if (parent.email) {
      try {
        ...
        emailStatus = error ? "failed" : "sent";
        emailMeta = data?.id ? { resend_id: data.id } : {};
      } catch (err) {
        emailMeta = { error: String(err) };
      }
    }
