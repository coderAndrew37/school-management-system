...
type BadgeVariant = 'amber' | 'emerald' | 'rose' | 'sky' | 'slate';
...
export function Badge({ variant, children, className = '' }: BadgeProps): React.ReactElement {
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium tracking-wide ${badgeStyles[variant]} ${className}`}
    >
      {children}
    </span>
  );
...
export function StatusBadge({ status }: StatusBadgeProps): React.ReactElement {
  const map: Record<'active' | 'transferred' | 'suspended', BadgeVariant> = {
    active: 'emerald',
    transferred: 'rose',
    suspended: 'amber',
  };
...
export function Modal({ isOpen, onClose, title, children, width = 'max-w-xl' }: ModalProps): React.ReactElement | null {
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <div
        className={`relative w-full ${width} bg-[#0e1220] border border-white/[0.09] rounded-2xl shadow-2xl`}
...
export function SlideOver({ isOpen, onClose, title, subtitle, children }: SlideOverProps): React.ReactElement {
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 h-full z-50 w-full max-w-lg bg-[#0e1220] border-l border-white/[0.08] shadow-2xl transition-transform duration-300 ease-in-out flex flex-col ${
  ...
  return (
    <div>
      <input
        ...
        } rounded-xl px-3.5 py-2.5 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-amber-400/40 focus:ring-1 focus:ring-amber-400/20 transition-colors ${className}`}
      />
  ...
  return (
    <div>
      <textarea
        ...
        className={`w-full bg-white/[0.04] border ${
          error ? 'border-rose-500/50' : 'border-white/[0.08]'
        } rounded-xl px-3.5 py-2.5 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-amber-400/40 focus:ring-1 focus:ring-amber-400/20 transition-colors resize-none ${className}`}
      />
      {error && <p className="mt-1 text-rose-400 text-xs">{error}</p>}
    </div>
  );
...
export function Button({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed ${buttonVariants[variant]} ${sizeStyles[size]} ${className}`}
    >
      {isLoading && (
