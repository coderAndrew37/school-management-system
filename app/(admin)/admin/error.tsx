...
import { GraduationCap, AlertCircle, RefreshCw, LayoutDashboard } from "lucide-react";
...
export default function AdminGlobalError({ error, reset }: GlobalErrorProps) {
  useEffect(() => {
    console.error("[Kibali Core Dashboard Exception]:", error);
  }, [error]);

  return (
    <div className="w-full min-h-[75vh] flex items-center justify-center p-4">
      {/* Background Ambient Mask */}
      ...
      <div className="w-full max-w-md text-center space-y-6">
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-xl shadow-2xl overflow-hidden">
          {/* Top visual threat warning vector line strip */}
          ...
          <div className="p-7 space-y-5">
            ...
            <div>
              <h2 className="text-base font-bold text-white tracking-tight mb-1.5">
                Execution Pipeline Collapse
              </h2>
              <p className="text-xs text-white/50 leading-relaxed max-w-xs mx-auto">
                An unhandled structural exception occurred while parsing records. The requested database operations could not complete.
              </p>
            </div>

            {/* Error Diagnostics Fingerprint Block */}
            <div className="rounded-xl border border-white/[0.05] bg-white/[0.02] px-4 py-3 text-left space-y-1">
              ...
              <p className="text-xs font-mono text-rose-300/80 break-words line-clamp-2">
                {error.message || "Unknown operational error context signature."}
              </p>
            </div>
        ...
        {error.digest && (
          <p className="text-[9px] font-mono text-white/20 select-all tracking-tight">
            Transaction Error Token: {error.digest}
          </p>
        )}
      </div>
    </div>
