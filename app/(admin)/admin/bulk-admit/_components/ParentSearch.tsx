...
export function ParentSearch({ selected, onSelect }: ParentSearchProps) {
  ...
  const { results, loading, open, close, open: openDropdown } = useParentSearch(query);
        ...
        <div className="flex-1 min-w-0">
          ...
          {selected.children.length > 0 && (
            <p className="text-[11px] text-emerald-400/60 mt-0.5">
              {selected.children.length} existing child
  ...
  return (
    <div ref={ref} className="relative" onBlur={handleBlur}>
      <div className="relative">
        <Search
          className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-white/25 pointer-events-none"
          aria-hidden="true"
        />
        <input
          type="text"
        ...
        {loading && (
          <Loader2
            className="absolute right-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-white/25 animate-spin"
      ...
      {open && results.length > 0 && (
        <ul
          ...
          role="listbox"
          aria-label="Parent search results"
          ...
          {results.map((p) => (
            <li key={p.id} role="option" aria-selected="false">
              <button
                ...
                onMouseDown={() => { onSelect(p); close(); setQuery(""); }}
              ...
              >
                <div
                  className="h-8 w-8 rounded-lg bg-amber-400/10 flex items-center justify-center shrink-0"
                  aria-hidden="true"
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {p.children.length > 0 && (
                    <p className="text-[11px] text-white/25 mt-0.5">
                      Children:{" "}
                      {p.children.map((c) => `${c.full_name} (${c.current_grade})`).join(", ")}
                    </p>
                  )}
                </div>
              </button>
      ...
      {open && !loading && results.length === 0 && query.length >= 2 && (
        <div
          role="status"
          aria-live="polite"
          className="absolute z-50 top-full mt-1.5 left-0 right-0 rounded-xl border border-white/10 bg-[#101525] px-4 py-3"
        >
          <p className="text-sm text-white/40">No parents found for &ldquo;{query}&rdquo;</p>
